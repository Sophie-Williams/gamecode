        ...
        foreach my $snaffle_id ( sort keys %{ $entity{'SNAFFLE'} } ) {
            ...
            if ($distance2SNAFFLE) {
                if ( $distance2SNAFFLE == $nearest ) {
                    $entity{'WIZARD'}{$wizard_id}{SNAFFLE}{$snaffle_id}{target}
                      = "true";
...
sub catcher_snafflecheck {
    ...
    if ( $entity{'SNAFFLE'} ) {
        ...
        foreach my $snaffle_id ( sort keys %{ $entity{'SNAFFLE'} } ) {
            ...
            my $nearest = min map { $snaffles->{$_}{distance_enemybase} }
              keys %$snaffles;
...
sub defender_snafflecheck {
    ...
    if ( $entity{'SNAFFLE'} ) {
        ...
        foreach my $snaffle_id ( sort keys %{ $entity{'SNAFFLE'} } ) {
            ...
            my $snaffles = $entity{'SNAFFLE'};
            my $nearest = min map { $snaffles->{$_}{distance_mybase} }
              keys %$snaffles;
...
sub opponent_wizard_check {
    ...
    if ( $entity{'OPPONENT_WIZARD'} ) {
        ...
        foreach
          my $opponent_wizard_id ( sort keys %{ $entity{'OPPONENT_WIZARD'} } )
        {
            ...
            if ($distance2opponent_wizard) {
                if ( $distance2opponent_wizard == $nearest ) {
                    ...
                    $entity{'OPPONENT_WIZARD'}{$opponent_wizard_id}{tracked} =
                      "true";
                    return (
                        "true",
                        $entity{'WIZARD'}{$wizard_id}{OPPONENT_WIZARD}
        ...
        foreach my $bludger_id ( sort keys %{ $entity{'BLUDGER'} } ) {
            ...
            if ( $distance2BLUDGER < 1000 ) {
                if ( $distance2BLUDGER == $nearest ) {
                    $entity{'WIZARD'}{$wizard_id}{BLUDGER}{$bludger_id}{target}
                      = "true";
    ...
    if ( $entity{'OPPONENT_WIZARD'} ) {
          ...
          my $opponent_wizard_id ( sort keys %{ $entity{'OPPONENT_WIZARD'} } )
        {
                ...
                and ( $entity{'OPPONENT_WIZARD'}{$enemy_id_2}{position} ) )
            {
                if (
                    (
                        $entity{'OPPONENT_WIZARD'}{$enemy_id_1}{position} eq
                        'good'
                    )
                    and
                    ( $entity{'OPPONENT_WIZARD'}{$enemy_id_2}{position} eq
                        'good' )
                  )
                {
                    $attackmode = "true";
    ...
    if ( $action eq "catcher" ) {
        ...
        if ( ($handle_snaffle) and ( $handle_snaffle eq 'true' ) ) {
            ...
            if (
                ...
                and (
                    (
                        $entity{'WIZARD'}{$wizard_id}{angle_enemybase_diff} < 15
                    )
                    or ( $entity{'WIZARD'}{$wizard_id}{angle_enemybase_diff} >
                        -15 )
                )
              )
            {
                if (
                    ( $enemy_side eq "right" )
                    and ( $entity{'WIZARD'}{$wizard_id}{SNAFFLE}{$snaffle_id}{x}
                        > $entity{'WIZARD'}{$wizard_id}{x} )
                  )
                {
            ...
            and ( $distance2BLUDGER < 2000 ) )
        {
                ...
                and ( $distance2SNAFFLE < 3500 ) )
            {
                ...
                (
                    $entity{'WIZARD'}{$wizard_id}{SNAFFLE}{$snaffle_id}{vx} == 0
                )
                and
                ( $entity{'WIZARD'}{$wizard_id}{SNAFFLE}{$snaffle_id}{vy} == 0 )
                and ( $entity{'WIZARD'}{$wizard_id}{distance_mybase} < 3000 )
            ...
            elsif ( ( $entity{'WIZARD'}{$wizard_id}{distance_mybase} < 5000 )
                and ( $distance2SNAFFLE < 5000 ) )
            {
                ...
                and ( $entity{'WIZARD'}{$wizard_id}{distance_mybase} < 2500 ) )
            {
                print "MOVE "
                  . $entity{'WIZARD'}{$wizard_id}{OPPONENT_WIZARD}
                  {$opponent_wizard_id}{x} . " "
...
while (1) {
    ...
    for my $i ( 0 .. $entities - 1 ) {
        ...
        $entity{$entity_type}{$entity_id}{distance_mybase} = &getdistance(
            $entity{$entity_type}{$entity_id}{x},
            $entity{$entity_type}{$entity_id}{y},
            $mybase_x, $mybase_y
        );
        $entity{$entity_type}{$entity_id}{distance_enemybase} = &getdistance(
            $entity{$entity_type}{$entity_id}{x},
            $entity{$entity_type}{$entity_id}{y},
            $enemybase_x, $enemybase_y
        );
    ...
    foreach my $wizard_id ( sort keys %{ $entity{'WIZARD'} } ) {
        ...
        foreach my $snaffle_id ( sort keys %{ $entity{'SNAFFLE'} } ) {
            ...
            $entity{'WIZARD'}{$wizard_id}{SNAFFLE}{$snaffle_id}{angle_WIZARD} =
              &getangle(
                $entity{'WIZARD'}{$wizard_id}{x},
                $entity{'WIZARD'}{$wizard_id}{x},
                $entity{'SNAFFLE'}{$snaffle_id}{x},
            ...
            $entity{'WIZARD'}{$wizard_id}{SNAFFLE}{$snaffle_id}{angle_mybase} =
              &getangle(
                $entity{'SNAFFLE'}{$snaffle_id}{x},
                $entity{'SNAFFLE'}{$snaffle_id}{y},
                $mybase_x, $mybase_y
              );
            $entity{'WIZARD'}{$wizard_id}{SNAFFLE}{$snaffle_id}{angle_enemybase}
              = &getangle(
                $entity{'SNAFFLE'}{$snaffle_id}{x},
                $entity{'SNAFFLE'}{$snaffle_id}{y},
                $enemybase_x, $enemybase_y
            ...
            $entity{'WIZARD'}{$wizard_id}{SNAFFLE}{$snaffle_id}{distance} =
              &getdistance(
                $entity{'WIZARD'}{$wizard_id}{x},
                $entity{'WIZARD'}{$wizard_id}{y},
                $entity{'SNAFFLE'}{$snaffle_id}{x},
            ...
            if ( $wizard_id == 0 ) {
                print STDERR "COLLISION: "
                  . $entity{'SNAFFLE'}{$snaffle_id}{collide_mybase} . " \n";
            }
        }
        ...
        foreach my $bludger_id ( sort keys %{ $entity{'BLUDGER'} } ) {
            ...
            $entity{'WIZARD'}{$wizard_id}{BLUDGER}{$bludger_id}{angle_WIZARD} =
              &getangle(
                $entity{'WIZARD'}{$wizard_id}{x},
                $entity{'WIZARD'}{$wizard_id}{x},
                $entity{'BLUDGER'}{$bludger_id}{x},
            ...
            $entity{'WIZARD'}{$wizard_id}{BLUDGER}{$bludger_id}{angle_mybase} =
              &getangle(
                $entity{'BLUDGER'}{$bludger_id}{x},
                $entity{'BLUDGER'}{$bludger_id}{y},
                $mybase_x, $mybase_y
              );
            $entity{'WIZARD'}{$wizard_id}{BLUDGER}{$bludger_id}{angle_enemybase}
              = &getangle(
                $entity{'BLUDGER'}{$bludger_id}{x},
                $entity{'BLUDGER'}{$bludger_id}{y},
                $enemybase_x, $enemybase_y
            ...
            $entity{'WIZARD'}{$wizard_id}{BLUDGER}{$bludger_id}{distance} =
              &getdistance(
                $entity{'WIZARD'}{$wizard_id}{x},
                $entity{'WIZARD'}{$wizard_id}{y},
                $entity{'BLUDGER'}{$bludger_id}{x},
